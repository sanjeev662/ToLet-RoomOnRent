...
export default function PlaceGallery({ place }) {
  ...
  if (showAllPhotos) {
    return (
      <div className="absolute inset-0 bg-black text-white min-h-screen pt-8">
        <div className="bg-black p-8 grid gap-4">
          <div>
            <h2 className="text-3xl mr-48">Photos of {place.title}</h2>
            <button
              onClick={() => setShowAllPhotos(false)}
              className="fixed right-12 top-12 flex gap-1 py-2 px-4 rounded-2xl shadow shadow-black bg-white text-black"
            >
              <svg
